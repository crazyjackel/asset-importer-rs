...
use gltf::{json::{
    buffer::View, extensions::material::Material as ExtensionMaterial, image::MimeType, material::{AlphaCutoff, EmissiveFactor, NormalTexture, OcclusionTexture, StrengthFactor}, texture::{Info, Sampler}, validation::{Checked, USize64}, Image, Index, Material, Root, Texture
}, material::AlphaMode};

use crate::{
    core::error::AiExportError,
    structs::{matkey::{self, AI_MATKEY_BASE_COLOR, AI_MATKEY_COLOR_DIFFUSE, AI_MATKEY_COLOR_EMISSIVE, AI_MATKEY_COLOR_SPECULAR, AI_MATKEY_METALLIC_FACTOR, AI_MATKEY_OPACITY, AI_MATKEY_ROUGHNESS_FACTOR, AI_MATKEY_SHADING_MODEL, AI_MATKEY_SHININESS, AI_MATKEY_TWOSIDED, _AI_MATKEY_MAPPINGMODE_U_BASE, _AI_MATKEY_MAPPINGMODE_V_BASE, _AI_MATKEY_TEXTURE_BASE}, scene::AiScene, AiColor3D, AiColor4D, AiMaterial, AiPropertyTypeInfo, AiShadingMode, AiTextureType},
};

use super::{
    gltf2_exporter::{generate_unique_name, Gltf2Exporter, Output},
    gltf2_importer_material::{AI_MATKEY_GLTF_ALPHACUTOFF, AI_MATKEY_GLTF_PBRMETALLICROUGHNESS_METALLICROUGHNESS_TEXTURE, _AI_MATKEY_GLTF_MAPPINGFILTER_MAG_BASE, _AI_MATKEY_GLTF_MAPPINGFILTER_MIN_BASE, _AI_MATKEY_GLTF_MAPPINGID_BASE, _AI_MATKEY_GLTF_MAPPINGNAME_BASE, _AI_MATKEY_GLTF_SCALE_BASE},
};
impl Gltf2Exporter {
...
fn get_material_texture_occlusion(
    ...
    get_material_texture(
        ...
        OcclusionTexture { index: x.index, tex_coord: x.tex_coord, extensions: Default::default(), extras: x.extras, strength: StrengthFactor(strength) }
    })
}
...
fn handle_base(scene: &AiScene, root: &mut Root, buffer_data: &mut Vec<u8>, mut unique_names_map: &mut HashMap<String, u32>, 
    ...
    material.alpha_mode = ai_material
        ...
        .and_then(|info| match info {
            AiPropertyTypeInfo::Binary(binary) => {
                match String::from_utf8(binary.to_vec()).ok(){
...
#[cfg(not(feature = "KHR_materials_pbrSpecularGlossiness"))]
fn handle_specular_glossiness(_scene: &AiScene, _root: &mut Root, _buffer_data: &mut Vec<u8>, _unique_names_map: &mut HashMap<String, u32>, 
    _texture_name_to_index_map: &mut HashMap<String, u32>, _ai_material: &AiMaterial, _material: &mut Material, _is_binary: bool) {
}
#[cfg(feature = "KHR_materials_pbrSpecularGlossiness")]
...
fn handle_specular(
    ...
    use crate::structs::matkey::AI_MATKEY_SPECULAR_FACTOR;

    let color_specular_option = ai_material
        .get_property_ai_color_rgb(AI_MATKEY_COLOR_SPECULAR, Some(AiTextureType::None), 0);
    if color_specular_option.is_none(){
        return false;
    }
    let color_specular = color_specular_option.unwrap();
    let specular_factory_option = ai_material
        .get_property_ai_float(AI_MATKEY_SPECULAR_FACTOR, Some(AiTextureType::None), 0);
    ...
    material.specular = Some(Specular{
        ...
        specular_color_factor: SpecularColorFactor([color_specular.r,color_specular.g, color_specular.b]),
...
fn handle_transmission(
    ...
    let transmission_factor_option = ai_material
        .get_property_ai_float(AI_MATKEY_TRANSMISSION_FACTOR, Some(AiTextureType::None), 0);
...
fn handle_volume(
    ...
    use gltf::json::extensions::material::{AttenuationColor, AttenuationDistance, ThicknessFactor, Volume};

    use crate::structs::matkey::{AI_MATKEY_VOLUME_ATTENUATION_COLOR, AI_MATKEY_VOLUME_ATTENUATION_DISTANCE, AI_MATKEY_VOLUME_THICKNESS_FACTOR, AI_MATKEY_VOLUME_THICKNESS_TEXTURE};

    let thickness_option = ai_material
        .get_property_ai_float(AI_MATKEY_VOLUME_THICKNESS_FACTOR, Some(AiTextureType::None), 0);
    if thickness_option.is_none(){
        return false;
    }
    let thickness = thickness_option.unwrap();

    let attenuation_distance_option = ai_material
        .get_property_ai_float(AI_MATKEY_VOLUME_ATTENUATION_DISTANCE, Some(AiTextureType::None), 0);
    if attenuation_distance_option.is_none(){
        return false;
    }
    let attenuation_distance = attenuation_distance_option.unwrap();

    let attenuation_color_option = ai_material
        .get_property_ai_color_rgb(AI_MATKEY_VOLUME_ATTENUATION_COLOR, Some(AiTextureType::None), 0);
...
fn handle_ior(
    ...
    material.ior = ai_material
        ...
        .map(|x| Ior{ ior: IndexOfRefraction(x), extras: Default::default() });
    true
}
...
fn handle_emissive_strength(
    ...
    material.emissive_strength = ai_material
        ...
        .map(|x| EmissiveStrength{ emissive_strength: EmissiveStrengthFactor(x) });
    true
}