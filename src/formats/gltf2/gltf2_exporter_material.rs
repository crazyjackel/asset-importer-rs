...
use crate::{
    ...
    structs::{
        matkey::{
            self, AI_MATKEY_BASE_COLOR, AI_MATKEY_COLOR_DIFFUSE, AI_MATKEY_COLOR_EMISSIVE,
            AI_MATKEY_COLOR_SPECULAR, AI_MATKEY_METALLIC_FACTOR, AI_MATKEY_OPACITY,
            AI_MATKEY_ROUGHNESS_FACTOR, AI_MATKEY_SHADING_MODEL, AI_MATKEY_SHININESS,
            AI_MATKEY_TWOSIDED, _AI_MATKEY_MAPPINGMODE_U_BASE, _AI_MATKEY_MAPPINGMODE_V_BASE,
            _AI_MATKEY_TEXTURE_BASE,
        },
        scene::AiScene,
        AiColor3D, AiColor4D, AiMaterial, AiPropertyTypeInfo, AiShadingMode, AiTextureType,
    },
};
...
fn handle_pbr(
    ...
    material.pbr_metallic_roughness.base_color_factor = ai_material
        ...
        .and_then(|info| match info {
            AiPropertyTypeInfo::Binary(binary) => bytemuck::try_from_bytes::<AiColor4D>(binary)
                .ok()
...
fn handle_base(
    ...
    material.emissive_factor = ai_material
        ...
        .and_then(|info| match info {
            AiPropertyTypeInfo::Binary(binary) => bytemuck::try_from_bytes::<AiColor3D>(binary)
                .ok()
