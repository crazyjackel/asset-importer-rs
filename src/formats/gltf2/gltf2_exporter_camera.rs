...
use gltf::json::{camera::{Orthographic, Perspective, Type}, validation::Checked, Camera, Root};

use crate::{core::error::AiExportError, structs::scene::AiScene};
...
impl Gltf2Exporter{
    pub(crate) fn export_cameras(
        ...
        scene: &AiScene,
        root: &mut Root,
        unique_names_map: &mut HashMap<String, u32>) -> Result<HashMap<String, usize>, AiExportError> 
    {
        ...
        for (index, ai_camera) in scene.cameras.iter().enumerate(){
            let orthographic = if ai_camera.orthographic_width > 0.0{
                Some(Orthographic{
                    ...
                    ymag: if ai_camera.aspect_ratio != 0.0 { (1.0 / ai_camera.aspect_ratio) * ai_camera.orthographic_width } else { 1.0 },
            ...
            let perspective = if orthographic.is_some(){
                ...
                * f32::atan(
                ...
                Some(Perspective{
                    aspect_ratio: if ai_camera.aspect_ratio != 0.0 { Some(ai_camera.aspect_ratio) } else { None },
                    yfov,
            ...
            let camera = Camera{
                ...
                orthographic,
                perspective,
                type_: Checked::Valid(if is_some {Type::Orthographic} else {Type::Perspective}),
                extensions: Default::default(),
                extras: Default::default(),
            };
