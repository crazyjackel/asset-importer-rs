...
impl Gltf2Importer {
    pub fn from_source<R: Read + Seek, F: Fn(&Path) -> io::Result<R>>(
        ...
        #[cfg(feature = "gltf2_guess_mime_type")]
        let guess_format = |encoded_image: &[u8]| match image::guess_format(encoded_image) {
            Ok(image::ImageFormat::Png) => Some(image::ImageFormat::Png),
            Ok(image::ImageFormat::Jpeg) => Some(image::ImageFormat::Jpeg),
            Ok(image::ImageFormat::WebP) => Some(image::ImageFormat::WebP),
            _ => None,
        };
        #[cfg(not(feature = "gltf2_guess_mime_type"))]
...
fn test_gltf2_texture_import() {
    ...
    let (embedded_textures, _tex_ids) = Gltf2Importer::import_embedded_textures(
        &document,
        Some(exe_path),
        &super::gltf2_importer::default_file_loader,
        &[],
    )
    .unwrap();
